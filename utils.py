...
class Batchify:
    def __init__(self, data, word2idx, seq_len=15, batch_size=1, shuffle=False):
        bos = word2idx['<bos>']
        eos = word2idx['<eos>']
        pad = word2idx['<pad>']
        u, i, r, t, f = [], [], [], [], []
    ...
    def search(self,user_id,item_id):
        for a,b,c,d,e  in zip(self.u,self.i,self.r,self.t,self.f):
           if (str(a) == user_id) and (str(b) == item_id):
               ...
               seq = torch.tensor(mid4, dtype=torch.int64).contiguous()
